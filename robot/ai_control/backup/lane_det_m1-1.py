...
def detect_edges(frame):
    ...
    show_image('edged_img', edges_img)
...
def region_of_interest(canny):
    ...
    polygon = np.array([[
        (0, height * int(1 / 2)),     #sgkim
        (width, height * int(1 / 2)),
        (width, height),
        (0, height),
    ]], np.int32)
...
def compute_steering_angle(frame, lane_lines):
    ...
    if len(lane_lines) == 1:
        ...
        pre_offset = x2 - x1
        if x1 < 0:
            x_offset = x_offset + (x1 * 1.5)
        else:
            x_offset = pre_offset
        print(f'pre_offset, x_offset = {pre_offset}, {x_offset}')
